...
<script>
    ...
    function buildKelompokUI(item, statusLaporanDesa) {
        ...
        if (item.status === 'TTD_KETUA') {
            btnLihat = `<button onclick="window.location.href='?page=laporan_desa/lihat_laporan_kelompok&id=${item.id}'" class="bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors text-xs font-medium" title="Lihat Isi Laporan"><i class="fa-solid fa-eye mr-1"></i> Lihat Data</button>`;
        }

        if (item.status !== 'DRAFT' && statusLaporanDesa !== 'TTD_KETUA') {
            btnTolak = `<button onclick="tolakLaporan(${item.id}, '${toUcwords(item.nama_kelompok)}')" class="bg-red-50 text-red-700 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors text-xs font-medium" title="Tolak & Kembalikan ke Draft"><i class="fa-solid fa-rotate-left mr-1"></i> Tolak</button>`;
        } else if (item.status !== 'DRAFT' && statusLaporanDesa === 'TTD_KETUA') {
            btnTerkunci = `<span class="text-xs text-gray-400 bg-gray-50 px-2 py-1.5 rounded border border-gray-200 font-medium whitespace-nowrap" title="Terkunci: Laporan Desa telah disahkan"><i class="fa-solid fa-lock mr-1"></i> Terkunci</span>`;
        }

        // Susun Tombol menjadi satu container flex
        let actionBtn = `<span class="text-xs text-gray-400">-</span>`;
        const buttons = [btnLihat, btnTolak, btnTerkunci].filter(b => b !== '');

        if (buttons.length > 0) {
            actionBtn = `<div class="flex justify-end items-center gap-2 flex-wrap">${buttons.join('')}</div>`;
        }
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    if (res.status === 'success') {
                        ...
                        loadDetailKelompok(); // Refresh tabel dan cards
                    } else {
