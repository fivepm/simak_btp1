<?php
// === FILE FRONTEND: kelola_pembina.php ===
if (!isset($conn)) {
...
<div class="container mx-auto relative">
    ...
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-gray-700 text-2xl font-medium">Kelola Pembina</h3>
        <button id="tambahPembinaBtn" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 shadow transition transform hover:scale-105">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md overflow-x-auto relative min-h-[300px]">
        ...
        <div id="tableLoaderPembina" class="absolute inset-0 bg-white bg-opacity-80 z-10 flex justify-center items-center hidden">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="pembinaTableBody" class="bg-white divide-y divide-gray-200">
...
<!-- Modal Tambah -->
<div id="tambahPembinaModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        ...
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-11/12 max-w-sm sm:max-w-lg sm:w-full z-50">
            ...
            <form id="formTambahPembina" enctype="multipart/form-data">
                <input type="hidden" name="action" value="tambah_pembina">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Form Tambah Pembina</h3>
                    <div class="space-y-4">
                        <div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <input type="text" name="nomor_wa" id="tambah_nomor_wa_pembina" placeholder="Misal: 08123456789" class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-l-md border border-gray-300 focus:ring-2 focus:ring-green-500 outline-none sm:text-sm" required>
                                <button type="button" id="btnCekWaTambahPembina" class="inline-flex items-center px-4 py-2 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 text-gray-600 text-sm font-medium hover:bg-gray-100 focus:outline-none transition-colors">
                                    Tes Cek WA
                                </button>
                            </div>
                            <p id="msgCekWaTambahPembina" class="mt-1 text-xs hidden"></p>
                        </div>
                        <div>
...
<!-- Modal Edit -->
<div id="editPembinaModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        <div class="fixed inset-0 bg-gray-500 opacity-75 modal-backdrop"></div>
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-11/12 max-w-sm sm:max-w-lg sm:w-full z-50">
            <form id="formEditPembina" enctype="multipart/form-data">
                <input type="hidden" name="action" value="edit_pembina">
                <input type="hidden" name="edit_id" id="edit_id_pembina">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Form Edit Pembina</h3>
                    <div class="space-y-4">
                        <div>
                            <label for="edit_nama_pembina" class="block text-sm font-medium text-gray-700">Nama Lengkap*</label>
                            <input type="text" name="edit_nama" id="edit_nama_pembina" class="mt-1 w-full border border-gray-300 p-2 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Username (System Generated)</label>
                            <input type="text" id="view_username_pembina" class="mt-1 w-full border border-gray-200 bg-gray-100 text-gray-500 p-2 rounded-md cursor-not-allowed font-mono sm:text-sm" disabled>
                        </div>
                        <div>
                            ...
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <input type="text" name="edit_nomor_wa" id="edit_nomor_wa_pembina" class="flex-1 min-w-0 block w-full px-3 py-2 rounded-none rounded-l-md border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none sm:text-sm" required>
                                <button type="button" id="btnCekWaEditPembina" class="inline-flex items-center px-4 py-2 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 text-gray-600 text-sm font-medium hover:bg-gray-100 focus:outline-none transition-colors">
                                    Tes Cek WA
                                </button>
                            </div>
                            <p id="msgCekWaEditPembina" class="mt-1 text-xs hidden"></p>
                        </div>
                        <div>
                            ...
                            <div id="preview_ttd_lama_pembina" class="mt-2 text-sm text-gray-600 hidden flex items-center gap-2 bg-gray-50 p-2 rounded">
                                <span class="text-xs font-semibold">TTD Saat Ini:</span>
                                <img id="img_ttd_lama_pembina" src="" class="h-8 object-contain bg-white border rounded">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="edit_tingkat_pembina" class="block text-sm font-medium text-gray-700">Tingkat</label>
                                <select name="edit_tingkat" id="edit_tingkat_pembina" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none sm:text-sm" required>
                                    <option value="desa">Desa</option>
                                    <option value="kelompok">Kelompok</option>
                                </select>
                            </div>
                            <div>
                                <label for="edit_kelompok_pembina" class="block text-sm font-medium text-gray-700">Kelompok</label>
                                <select name="edit_kelompok" id="edit_kelompok_pembina" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none sm:text-sm" required>
                                    <option value="bintaran">Bintaran</option>
                                    <option value="gedongkuning">Gedongkuning</option>
                                    <option value="jombor">Jombor</option>
                                    <option value="sunten">Sunten</option>
...
<!-- Modal Hapus -->
<div id="hapusPembinaModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        <div class="fixed inset-0 bg-gray-500 opacity-75 modal-backdrop"></div>
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full z-50">
            <form id="formHapusPembina">
                <input type="hidden" name="action" value="hapus_pembina">
                <input type="hidden" name="hapus_id" id="hapus_id_pembina">
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Hapus Pembina</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">Anda yakin ingin menghapus pengguna <strong id="hapus_nama_pembina" class="text-gray-900"></strong>?<br>Tindakan ini hanya akan diarsipkan (Soft Delete).</p>
                            </div>
                        </div>
                    </div>
                </div>
...
<!-- Modal QR Code -->
<div id="qrCodeModalPembina" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        <div class="fixed inset-0 bg-gray-500 opacity-75 modal-backdrop"></div>
        <div class="bg-white rounded-lg text-center p-6 overflow-hidden shadow-xl transform transition-all sm:max-w-sm sm:w-full z-50">
            <h3 class="text-lg font-medium text-gray-900">QR Code untuk <span id="qr_nama_pembina" class="font-bold"></span></h3>
            <div id="qrcode-container-pembina" class="my-4 flex justify-center"></div>
            <a id="download-qr-link-pembina" href="#" download="qrcode.png" class="inline-block bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg">Download</a>
...
<div id="imageModalPembina" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 z-10 flex items-center justify-center p-4">
        <div class="relative bg-white rounded-lg shadow-xl overflow-hidden max-w-3xl w-full max-h-[90vh] flex flex-col z-50">
            ...
            <div class="flex items-center justify-center bg-gray-100 h-full w-full p-2" style="background-image: linear-gradient(45deg, #e5e5e5 25%, transparent 25%), linear-gradient(-45deg, #e5e5e5 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #e5e5e5 75%), linear-gradient(-45deg, transparent 75%, #e5e5e5 75%); background-size: 20px 20px; background-position: 0 0, 0 10px, 10px -10px, -10px 0px;">
                <img id="modalImageDisplayPembina" class="max-w-full max-h-[85vh] object-contain rounded" src="" alt="Preview TTD">
            </div>
        </div>
    </div>
</div>
...
<!-- MODAL RESET PIN -->
<div id="resetPinModalPembina" class="fixed z-50 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500 mb-4">
                                Anda akan mereset PIN untuk <strong id="reset_target_nama_pembina">User</strong> menjadi default.
                            </p>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Verifikasi PIN Superadmin:</label>
                            <input type="password" id="superadmin_pin_pembina" class="shadow-sm focus:ring-yellow-500 focus:border-yellow-500 block w-full sm:text-sm border-gray-300 rounded-md p-2 border" placeholder="Masukkan PIN Anda" inputmode="numeric">
                            <p id="reset_error_msg_pembina" class="text-red-600 text-xs mt-1 hidden"></p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" id="btnConfirmResetPembina" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-yellow-600 text-base font-medium text-white hover:bg-yellow-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                    Reset PIN
                </button>
...
<script>
    const userRoleSessionPembina = '<?= $_SESSION['user_role'] ?? 'admin' ?>';
</script>

<script>
    document.addEventListener('DOMContentLoaded', function() {
        const API_URL_PEMBINA = 'pages/master/ajax_kelola_pembina.php'; // Sesuaikan path

        // --- FLAG VALIDASI WA FRONTEND ---
        let isWaValidated_Tambah = false;
        let isWaValidated_Edit = true;

        document.getElementById('tambah_nomor_wa_pembina').addEventListener('input', function() {
            isWaValidated_Tambah = false;
            document.getElementById('msgCekWaTambahPembina').classList.add('hidden');
        });

        document.getElementById('edit_nomor_wa_pembina').addEventListener('input', function() {
            isWaValidated_Edit = false;
            document.getElementById('msgCekWaEditPembina').classList.add('hidden');
        });

        // --- MANAJEMEN MODAL ---
        const modals = {
            tambah: document.getElementById('tambahPembinaModal'),
            edit: document.getElementById('editPembinaModal'),
            hapus: document.getElementById('hapusPembinaModal'),
            reset: document.getElementById('resetPinModalPembina'),
            qr: document.getElementById('qrCodeModalPembina'),
            image: document.getElementById('imageModalPembina')
        };
        ...
        document.body.addEventListener('click', function(e) {
            if (e.target.closest('.modal-close-btn') || e.target.classList.contains('modal-backdrop')) {
                ...
                if (modals.image) document.getElementById('modalImageDisplayPembina').src = '';
            }
        });

        document.getElementById('tambahPembinaBtn').onclick = () => {
            document.getElementById('formTambahPembina').reset();
            document.getElementById('msgCekWaTambahPembina').classList.add('hidden');
            isWaValidated_Tambah = false;
        ...
        // --- LOAD DATA (GET) ---
        function loadData() {
            const loader = document.getElementById('tableLoaderPembina');
            loader.classList.remove('hidden');

            fetch(`${API_URL_PEMBINA}?action=get_data`)
                .then(res => {
                    hideGlobalOverlay();
        ...
        function renderTable(data) {
            const tbody = document.getElementById('pembinaTableBody');
            tbody.innerHTML = '';
            ...
            data.forEach(p => {
                ...
                // Badge Preview TTD
                let ttdHtml = p.ttd ?
                    `<button class="text-blue-500 hover:text-blue-700 font-medium btn-lihat-ttd-pembina flex flex-col items-center mx-auto" data-ttd="../uploads/ttd/${p.ttd}"><i class="fa-solid fa-signature text-lg"></i> <span class="text-[10px]">Lihat</span></button>` :
                    `<span class="text-xs text-gray-400 italic">Belum ada</span>`;

                const tr = document.createElement('tr');
                tr.innerHTML = `
                <td class="px-6 py-4 text-sm align-middle">${i++}</td>
                ...
                <td class="px-6 py-4 text-center align-middle">
                    <button class="qr-code-btn-pembina text-indigo-500 hover:text-indigo-700 font-medium flex flex-col items-center mx-auto"
                ...
                <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium align-middle">
                    <div class="flex flex-col gap-2 w-max mx-auto">
                        <button class="edit-btn-pembina text-left text-indigo-600 hover:text-indigo-900"
                        ...
                        <button class="hapus-btn-pembina text-left text-red-600 hover:text-red-900" data-id="${p.id}" data-nama="${p.nama}">
                            <i class="fa-solid fa-trash mr-1"></i> Hapus
                        </button>
                        ${userRoleSessionPembina === 'superadmin' ? `
                            <button class="reset-pin-btn-pembina text-left text-yellow-600 hover:text-yellow-800"
                                data-id="${p.id}" data-nama="${p.nama}" data-barcode="${p.barcode}" data-role="pembina">
        ...
        document.body.addEventListener('click', function(e) {
            ...
            // Lihat TTD Modal
            if (e.target.closest('.btn-lihat-ttd-pembina')) {
                const btn = e.target.closest('.btn-lihat-ttd-pembina');
                document.getElementById('modalImageDisplayPembina').src = btn.dataset.ttd;
                openModal(modals.image);
            }

            if (e.target.closest('.hapus-btn-pembina')) {
                const btn = e.target.closest('.hapus-btn-pembina');
                document.getElementById('hapus_id_pembina').value = btn.dataset.id;
                document.getElementById('hapus_nama_pembina').textContent = btn.dataset.nama;
                openModal(modals.hapus);
            }

            if (e.target.closest('.edit-btn-pembina')) {
                const btn = e.target.closest('.edit-btn-pembina');
                document.getElementById('msgCekWaEditPembina').classList.add('hidden');

                document.getElementById('formEditPembina').reset();

                document.getElementById('edit_id_pembina').value = btn.dataset.id;
                document.getElementById('edit_nama_pembina').value = btn.dataset.nama;
                document.getElementById('edit_nomor_wa_pembina').value = btn.dataset.nomor_wa;

                if (document.getElementById('view_username_pembina')) document.getElementById('view_username_pembina').value = btn.dataset.username;
                if (document.getElementById('edit_kelompok_pembina')) document.getElementById('edit_kelompok_pembina').value = btn.dataset.kelompok;
                if (document.getElementById('edit_tingkat_pembina')) document.getElementById('edit_tingkat_pembina').value = btn.dataset.tingkat;

                // Preview TTD Lama
                const ttdLamaDiv = document.getElementById('preview_ttd_lama_pembina');
                if (btn.dataset.ttd) {
                    document.getElementById('img_ttd_lama_pembina').src = `../uploads/ttd/${btn.dataset.ttd}`;
            ...
            if (e.target.closest('.qr-code-btn-pembina')) {
                const btn = e.target.closest('.qr-code-btn-pembina');
                openModal(modals.qr);
                const container = document.getElementById('qrcode-container-pembina');
                const downloadLink = document.getElementById('download-qr-link-pembina');
                container.innerHTML = '';
                document.getElementById('qr_nama_pembina').textContent = btn.dataset.nama;
                ...
                newDownloadLink.addEventListener('click', function() {
                    ...
                    let pesanDinamis = tingkatPemilik == 'desa' ?
                        `Mendownload file QR Code *${namaPemilik}* - Pembina Desa.` :
                        `Mendownload file QR Code *${namaPemilik}* - Pembina Kelompok ${kelompokPemilik}.`;
            ...
            if (e.target.closest('.reset-pin-btn-pembina')) {
                const btn = e.target.closest('.reset-pin-btn-pembina');
                targetUserIdPembina = btn.dataset.id;
                targetUserBarcodePembina = btn.dataset.barcode;
                targetUserRolePembina = btn.dataset.role;
                document.getElementById('reset_target_nama_pembina').textContent = btn.dataset.nama;

                document.getElementById('superadmin_pin_pembina').value = '';
                document.getElementById('reset_error_msg_pembina').classList.add('hidden');
                openModal(modals.reset);
                setTimeout(() => document.getElementById('superadmin_pin_pembina').focus(), 100);
            }
        });

        // --- CEK WA API (Manual Trigger) ---
        function setupCekWa(inputId, btnId, msgId, flagType) {
                ...
                fetch('pages/master/ajax_cek_wa.php', {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            ...
                            if (flagType === 'tambah') isWaValidated_Tambah = true;
                            if (flagType === 'edit') isWaValidated_Edit = true;
                        } else {
                            msgEl.className = "mt-1 text-xs font-medium text-red-600";
                            msgEl.innerHTML = '<i class="fa-solid fa-circle-xmark"></i> ' + data.pesan;
                            if (flagType === 'tambah') isWaValidated_Tambah = false;
                            if (flagType === 'edit') isWaValidated_Edit = false;
                        }
                    })
        ...
        setupCekWa('tambah_nomor_wa_pembina', 'btnCekWaTambahPembina', 'msgCekWaTambahPembina', 'tambah');
        setupCekWa('edit_nomor_wa_pembina', 'btnCekWaEditPembina', 'msgCekWaEditPembina', 'edit');
        ...
        function submitAjaxForm(e, manualData = null) {
            ...
            if (manualData) {
                ...
                if (actionType === 'tambah_pembina' && !isWaValidated_Tambah) {
                ...
                if (actionType === 'edit_pembina') {
                    const editWaVal = document.getElementById('edit_nomor_wa_pembina').value;
        ...
        document.getElementById('formTambahPembina').addEventListener('submit', submitAjaxForm);
        document.getElementById('formEditPembina').addEventListener('submit', submitAjaxForm);
        document.getElementById('formHapusPembina').addEventListener('submit', submitAjaxForm);

        // Fitur Khusus Reset PIN
        const btnConfirmReset = document.getElementById('btnConfirmResetPembina');
        const errorMsg = document.getElementById('reset_error_msg_pembina');
        let targetUserIdPembina = null,
            targetUserBarcodePembina = null,
            targetUserRolePembina = 'pembina';

        if (btnConfirmReset) {
            btnConfirmReset.addEventListener('click', function() {
                const adminPin = document.getElementById('superadmin_pin_pembina').value;
                if (!adminPin) {
                ...
                fetch('pages/master/ajax_reset_pin.php', {
                        ...
                        body: new URLSearchParams({
                            'target_id': targetUserIdPembina,
                            'target_barcode': targetUserBarcodePembina,
                            'target_role': targetUserRolePembina,
                            'admin_pin': adminPin
                        })
                    })
                    .then(res => res.json())
