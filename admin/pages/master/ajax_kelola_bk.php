<?php
// === FILE BACKEND: ajax_kelola_bk.php ===
...
if ($action === 'get_data') {
    $sql = "SELECT id, nama, nama_panggilan, username, kelompok, tingkat, role, barcode, nomor_wa, last_login 
            FROM users 
            WHERE role = 'bk' AND deleted_at IS NULL 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'tambah_bk') {
        ...
        try {
            ...
            $conn->commit();
            writeLog('INSERT', "Menambah BK Baru: $nama ($tingkat $kelompok).");
            echo json_encode(['status' => 'success', 'message' => 'Data BK berhasil ditambahkan.']);
        } catch (Exception $e) {
            $conn->rollback();
    ...
    if ($action === 'edit_bk') {
        ...
        if ($stmt->execute()) {
            writeLog('UPDATE', "Update BK ID $id ($nama).");
