<?php
// === FILE BACKEND: ajax_kelola_pembina.php ===
...
function uploadTTD($fileInputName)
{
    ...
    $newFileName    = 'ttd_pembina_' . uniqid() . '.png';
...
if ($action === 'get_data') {
    $sql = "SELECT id, nama, nama_panggilan, username, kelompok, tingkat, role, barcode, nomor_wa, last_login, ttd 
            FROM users 
            WHERE role = 'pembina' AND deleted_at IS NULL 
            ORDER BY nama ASC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'tambah_pembina') {
        ...
        try {
            ...
            $username = '';
            while ($is_exist) {
                $username   = 'pembina' . rand(1000, 9999) . date('s');
            ...
            $conn->commit();
            writeLog('INSERT', "Menambah Pembina Baru: $nama ($tingkat $kelompok).");
            echo json_encode(['status' => 'success', 'message' => 'Data Pembina berhasil ditambahkan.']);
        } catch (Exception $e) {
            $conn->rollback();
    ...
    if ($action === 'edit_pembina') {
        ...
        if ($stmt->execute()) {
            writeLog('UPDATE', "Update Pembina ID $id ($nama).");
    ...
    // --- HAPUS PEMBINA (SOFT DELETE) ---
    if ($action === 'hapus_pembina') {
        $id   = $_POST['hapus_id'] ?? 0;
        $stmt = $conn->prepare("UPDATE users SET deleted_at = NOW() WHERE id = ?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute()) {
            writeLog('DELETE', "Menghapus (Soft) Pembina ID: $id");
