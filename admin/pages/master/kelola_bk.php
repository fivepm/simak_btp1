...
<div class="container mx-auto relative">
    ...
    <div class="flex justify-between items-center mb-6">
        <h3 class="text-gray-700 text-2xl font-medium">Kelola BK</h3>
        <button id="tambahBkBtn" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 shadow transition transform hover:scale-105">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md overflow-x-auto relative min-h-[300px]">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="bkTableBody" class="bg-white divide-y divide-gray-200">
...
<!-- Modal Tambah -->
<div id="tambahBkModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        ...
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-11/12 max-w-sm sm:max-w-lg sm:w-full z-50">
            <form id="formTambahBk">
                <input type="hidden" name="action" value="tambah_bk">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Form Tambah BK</h3>
                    <div class="space-y-4">
                        <div>
...
<!-- Modal Edit -->
<div id="editBkModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        <div class="fixed inset-0 bg-gray-500 opacity-75 modal-backdrop"></div>
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-11/12 max-w-sm sm:max-w-lg sm:w-full z-50">
            <form id="formEditBk">
                <input type="hidden" name="action" value="edit_bk">
                <input type="hidden" name="edit_id" id="edit_id">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Form Edit BK</h3>
                    <div class="space-y-4">
                        <div>
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-semibold leading-6 text-gray-900" id="modal-title">Hapus BK</h3>
                            <div class="mt-2">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const API_URL = 'pages/master/ajax_kelola_bk.php';
        ...
        // --- MANAJEMEN MODAL ---
        const modals = {
            tambah: document.getElementById('tambahBkModal'),
            edit: document.getElementById('editBkModal'),
            hapus: document.getElementById('hapusBkModal'),
        ...
        document.getElementById('tambahBkBtn').onclick = () => {
            document.getElementById('formTambahBk').reset();
            document.getElementById('msgCekWaTambah').classList.add('hidden');
            isWaValidated_Tambah = false;
        ...
        function renderTable(data) {
            ...
            data.forEach(p => {
                ...
                <td class="px-6 py-4 align-middle font-mono text-sm">${p.username}</td>
                    ...
                    <div class="flex flex-col gap-2 w-max mx-auto">
                        ...
                        ${userRoleSession === 'superadmin' ? `
                            <button class="reset-pin-btn text-left text-yellow-600 hover:text-yellow-800"
                                data-id="${p.id}" data-nama="${p.nama}" data-barcode="${p.barcode}" data-role="bk">
            ...
            if (e.target.closest('.qr-code-btn')) {
                ...
                newDownloadLink.addEventListener('click', function() {
                    ...
                    let pesanDinamis = tingkatPemilik == 'desa' ?
                        `Mendownload file QR Code *${namaPemilik}* - BK Desa.` :
                        `Mendownload file QR Code *${namaPemilik}* - BK Kelompok ${kelompokPemilik}.`;
        ...
        function submitAjaxForm(e, manualData = null) {
            ...
            if (manualData) {
                ...
                if (actionType === 'tambah_bk' && !isWaValidated_Tambah) {
        ...
        document.getElementById('formTambahBk').addEventListener('submit', submitAjaxForm);
        document.getElementById('formEditBk').addEventListener('submit', submitAjaxForm);
        document.getElementById('formHapusBk').addEventListener('submit', submitAjaxForm);
