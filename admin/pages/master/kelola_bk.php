...
$redirect_url = '?page=master/kelola_bk'; // Variabel untuk menyimpan URL redirect
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'tambah_bk') {
        ...
        if (empty($error_message)) {
            ...
            $username = '';
            while ($is_exist) {
                $username = 'bk' . rand(1000, 9999) . date('s');
            ...
            $role = 'bk'; // Role spesifik untuk BK
            $barcode = 'BK-' . uniqid(); // Prefix barcode baru
            ...
            if ($stmt->execute()) {
                // === CCTV ===
                if ($tingkat == 'desa') {
                    $desc_log = "Menambahkan *BK " . ucwords($tingkat) .  "* Baru : *" . ucwords($nama) . "*.";
                } else {
                    $desc_log = "Menambahkan *BK " . ucwords($tingkat) .  "* Baru : *" . ucwords($nama) . "* (Kelompok " . ucwords($kelompok) . ").";
                }
    ...
    if ($action === 'edit_bk') {
        ...
        if (empty($error_message)) {
            ...
            if ($stmt->execute()) {
                // === CCTV ===
                if ($tingkat == 'desa') {
                    $desc_log = "Memperbarui data *BK " . ucwords($tingkat) .  "*: *" . ucwords($nama) . "*.";
                } else {
                    $desc_log = "Memperbarui data *BK " . ucwords($tingkat) .  "*: *" . ucwords($nama) . "* (Kelompok " . ucwords($kelompok) . ").";
                }
    ...
    if ($action === 'hapus_bk') {
        ...
        if (empty($id)) {
            ...
            $stmt = $conn->prepare("DELETE FROM users WHERE id = ? AND role = 'bk'");
            $stmt->bind_param("i", $id);
            if ($stmt->execute()) {
                // === CCTV ===
                if ($ketuapjp['tingkat'] == 'desa') {
                    $desc_log = "Menghapus data *BK " . ucwords($ketuapjp['tingkat']) .  "*: *" . ucwords($ketuapjp['nama']) . "*.";
                } else {
                    $desc_log = "Menghapus data *BK " . ucwords($ketuapjp['tingkat']) .  "*: *" . ucwords($ketuapjp['nama']) . "* (Kelompok " . ucwords($ketuapjp['kelompok']) . ").";
                }
...
$pjp_users = [];
$sql = "SELECT id, nama, username, kelompok, tingkat, role, barcode FROM users WHERE role = 'bk' ORDER BY tingkat, nama ASC";
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody id="pjpTableBody" class="bg-white divide-y divide-gray-200">
                <?php if (empty($pjp_users)): ?>
                    <tr>
                        <td colspan="5" class="text-center py-4">Belum ada data BK.</td>
                    </tr>
                <?php else: ?>
                    <?php $i = 1;
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium">
                                ...
                                <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'superadmin'): ?>
                                    <button class="reset-pin-btn text-yellow-600 hover:text-yellow-800 ml-4"
                                        ...
                                        data-nama="<?php echo htmlspecialchars($user['nama']); ?>"
                                        data-barcode="<?php echo htmlspecialchars($user['barcode']); ?>"
                                        data-role="bk"> <!-- Target Role diset Guru -->
...
<div id="tambahBkModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        ...
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-11/12 max-w-sm sm:max-w-lg sm:w-full">
            <form method="POST" action="?page=master/kelola_bk">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <?php if (!empty($error_message) && $_SERVER['REQUEST_METHOD'] === 'POST'): ?>
            <?php if ($_POST['action'] === 'tambah_bk'): ?>
                openModal('tambahBkModal');
            <?php elseif ($_POST['action'] === 'edit_bk'): ?>
                openModal('editBkModal');
