...
<script>
    ...
    function renderRekapKelompok(kelasArray) {
        ...
        // Hanya kelas yang PUNYA DATA yang masuk ke dalam pembagi (average)
        // Kelas dengan kehadiran/ketercapaian = null dilewati (tidak ada siswa / tidak ada rekap)
        let totalHadir = 0,
            totalIzin = 0,
            totalSakit = 0,
            totalAlpa = 0;
        ...
        kelasArray.forEach(k => {
            // kehadiran: objek dengan hadir=null berarti tidak ada data presensi
        ...
        // Hasil null jika tidak ada satupun kelas yang memiliki data
        const avgHadir = countKehadiran > 0 ? Math.round(totalHadir / countKehadiran) : null;
        const avgIzin = countKehadiran > 0 ? Math.round(totalIzin / countKehadiran) : null;
        const avgSakit = countKehadiran > 0 ? Math.round(totalSakit / countKehadiran) : null;
        const avgAlpa = countKehadiran > 0 ? Math.round(totalAlpa / countKehadiran) : null;
        const avgCapaian = countCapaian > 0 ? Math.round(totalCapaian / countCapaian) : null;

        // Helper: format nilai persentase, tampilkan N/A jika null
        ...
        const strokeColor = avgCapaian !== null ? '#3b82f6' : '#d1d5db';
        ...
        container.innerHTML = `
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center">
                ...
                <p class="text-xs text-gray-500 mt-3 text-center bg-gray-50 px-3 py-2 rounded-full border border-gray-100">
                    <i class="fa-solid fa-circle-info text-primary mr-1"></i> ${keteranganKehadiran}
                </p>
            </div>
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center items-center">
                ...
                <div class="relative flex items-center justify-center mb-2">
                    ...
                    <span class="absolute text-4xl font-black text-primary">${capaianLabel}</span>
                </div>
                <p class="text-xs text-gray-500 mt-4 text-center bg-gray-50 px-4 py-2 rounded-full border border-gray-100">
                    <i class="fa-solid fa-circle-info text-primary mr-1"></i> ${keteranganCapaian}
                </p>
            </div>
        `;
    }
    ...
    function renderDetailKelas(kelasArray) {
        ...
        kelasArray.forEach((k, index) => {
            // --- Kehadiran ---
            // null = tidak ada siswa atau belum ada rekap presensi sama sekali
            ...
            // --- Ketercapaian ---
            // null = tidak ada siswa atau tidak ada target pembelajaran
                ...
                <div class="border rounded-xl p-5 bg-gray-50/50">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                            ...
                            <div class="mb-2">
                                <div class="flex justify-between text-xs mb-1"><span>Rata-rata Kelas</span><span class="${ketercapaianGlobalClass}">${ketercapaianGlobalDisplay}</span></div>
                                ${progressBar}
                            </div>
