...
if ($action === 'get_kesiapan_kelas') {
    ...
    $kelompok_filter = ($ketuapjp_level === 'kelompok' && in_array(strtolower($ketuapjp_kelompok), $kelompok_list))
        ? [strtolower($ketuapjp_kelompok)]
        : $kelompok_list;
    ...
    foreach ($kelompok_filter as $kel) {
        ...
        foreach ($kelas_list as $kls) {
            ...
            if ($pid) {
                ...
                $stmt_j->execute();
                $has_jadwal = ($stmt_j->get_result()->fetch_assoc()['cnt'] > 0);
                $stmt_j->close();
                ...
                $stmt_t->execute();
                $has_target = ($stmt_t->get_result()->fetch_assoc()['cnt'] > 0);
                $stmt_t->close();
            }
            $result[$kel][$kls] = ['jadwal' => $has_jadwal, 'target' => $has_target];
        }
    }
...
$kelompok_list = ['bintaran', 'gedongkuning', 'jombor', 'sunten'];
$kelas_list = ['paud', 'caberawit a', 'caberawit b', 'pra remaja', 'remaja', 'pra nikah'];
$kelompok_filter = ($ketuapjp_level === 'kelompok' && in_array(strtolower($ketuapjp_kelompok), $kelompok_list)) ? [strtolower($ketuapjp_kelompok)] : $kelompok_list;
...
$where_admin = ($ketuapjp_level === 'kelompok') ? " AND kelompok = '$ketuapjp_kelompok' " : "";
$where_admin_guru = ($ketuapjp_level === 'kelompok') ? " AND g.kelompok = '$ketuapjp_kelompok' " : "";
$where_admin_jp = ($ketuapjp_level === 'kelompok') ? " AND jp.kelompok = '$ketuapjp_kelompok' " : "";
...
try {
    ...
    if ($periode_aktif_id) {
        ...
        // =========================================================
        $absen_raw = [];
        $where_admin_alias = ($ketuapjp_level === 'kelompok') ? " AND p.kelompok = '$ketuapjp_kelompok' " : "";
        $sql_absen = "
