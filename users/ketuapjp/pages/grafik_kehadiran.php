...
if ($ketuapjp_tingkat === 'kelompok' && !empty($ketuapjp_kelompok) && in_array($ketuapjp_kelompok, $URUTAN_KELOMPOK_GLOBAL)) {
    $kelompok_to_display = [$ketuapjp_kelompok]; // Array berisi 1 kelompok
} else {
...
if ($selected_periode_id && !empty($kelompok_to_display)) { // Hanya jalankan jika ada kelompok yg boleh dilihat
    ...
    if ($ketuapjp_tingkat === 'kelompok' && !empty($ketuapjp_kelompok)) {
        $admin_where_clause = " AND p.kelompok = ? "; // Filter kelompok
        $bind_types .= "s"; // Tambah tipe string
        $bind_params[] = $ketuapjp_kelompok; // Tambah nilai kelompok
    }
    // =============================================
