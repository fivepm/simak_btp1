...
<script>
    ...
    // --- UBAH DI SINI: Terima parameter laporanId langsung (sesuai kodemu) ---
    ...
    function buildExportDesaUI(item) {
        ...
        if (!item.is_generated) {
            actionBtn = `<span class="text-xs text-gray-400 italic whitespace-nowrap">Laporan Belum Dibuka</span>`;
        } else {
            // Jika 4/4 Kelompok sudah Selesai
            if (!item.status_desa || item.status_desa === 'DRAFT' || item.status_desa === 'FINAL') {
                actionBtn = `<span class="text-xs text-gray-400 italic whitespace-nowrap" title="Hanya bisa diekspor jika sudah disahkan"><i class="fa-solid fa-file-pdf text-red-500/50 mr-2"></i> Ekspor PDF</span>`;
            } else if (item.status_desa === 'TTD_KETUA') {
                actionBtn = `<button type="button" onclick="exportPDF(${item.id})" class="bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-medium px-4 py-2 rounded-lg shadow-sm transition-colors whitespace-nowrap mr-2">
        ...
        let statusBadge = `<span class="bg-gray-100 text-gray-600 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap italic">Belum Dibuat</i></span>`;
        if (item.status_desa === 'DRAFT')
            ...
            statusBadge = `<span class="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-bold animate-pulse whitespace-nowrap"><i class="fa-solid fa-clock mr-1"></i> MENUNGGU TTD KETUA</span>`;
        else if (item.status_desa === 'TTD_KETUA')
        ...
        if (item.is_generated) {
            ...
            syaratBadge = `<div class="inline-flex items-center gap-2 whitespace-nowrap">
    ...
    function renderTableDesa(data) {
        ...
        data.forEach(item => {
            ...
            tr.innerHTML = `
                <td class="p-4 font-bold text-gray-800 whitespace-nowrap">${item.nama_periode}</td>
                <td class="p-4 text-center">${syaratBadge}</td>
                <td class="p-4 text-center">${item.is_generated ? statusBadge : '<span class="text-gray-300 italic">Belum Dibuka</span>'}</td>
    ...
    function renderCardsDesa(data) {
        ...
        data.forEach(item => {
            ...
            const displayStatusDesa = (item.is_generated) ? statusBadge : '<span class="text-gray-300 italic text-xs">Belum Dibuka</span>';
