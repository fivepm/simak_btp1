    ...
    function buildExportKelompokUI(item) {
        ...
        if (!item.status_laporan) {
            ...
            actionBtn = isPastEndDate ?
                `<span class="text-xs text-red-400 italic whitespace-nowrap">Menunggu akses dari Desa</span>` :
                `<span class="text-xs text-gray-400 italic whitespace-nowrap">Periode belum berakhir</span>`;
        } else {
            if (item.status_laporan === 'DRAFT') {
                ...
                // --- UBAH DI SINI: Kirim item.laporan_id (sesuai file terbarumu) ---
                actionBtn = `<button type="button" onclick="exportPDF(${item.laporan_id})" class="bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 font-medium px-4 py-2 rounded-lg shadow-sm transition-colors whitespace-nowrap mr-2">
