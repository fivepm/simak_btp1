...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $redirect_url = '?page=catatan_bk&peserta_id=' . $peserta_id_form;
    ...
    if ($action === 'tambah_catatan') {
        ...
        if (empty($error_message)) {
            ...
            $stmt->bind_param("isssi", $peserta_id_form, $tanggal_catatan, $permasalahan, $tindak_lanjut, $guru_id);
            if ($stmt->execute()) {
...
if ($selected_peserta_id) {
    $sql_catatan = "SELECT cb.*, COALESCE(u.nama, g.nama) as nama_pencatat 
                    FROM catatan_bk cb 
                    LEFT JOIN users u ON cb.dicatat_oleh_user_id = u.id
                    LEFT JOIN guru g ON cb.dicatat_oleh_user_id = g.id
                    WHERE cb.peserta_id = ? 
...
<div class="container mx-auto space-y-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <h3 class="text-xl font-medium text-gray-800 mb-4">Pilih Peserta di Kelas Anda</h3>
...
<div id="formModal" class="fixed z-20 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen">
        ...
        <div class="bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
            <form method="POST" action="?page=catatan_bk&peserta_id=<?php echo $selected_peserta_id; ?>">
