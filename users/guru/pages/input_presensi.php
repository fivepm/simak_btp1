...
if ($jadwal_id === 0) {
    echo '<div class="bg-red-100 p-4 rounded-lg">ID Jadwal tidak valid.</div>';
    return;
}

// Ambil data guru yang login
$guru_id = $_SESSION['user_id'] ?? 0;
$guru_kelompok = $_SESSION['user_kelompok'] ?? '';
$guru_kelas = $_SESSION['user_kelas'] ?? '';

// Ambil data jadwal dan pastikan guru ini berhak mengaksesnya
$jadwal = $conn->query("SELECT * FROM jadwal_presensi WHERE id = $jadwal_id")->fetch_assoc();

// KEAMANAN: Cek apakah jadwal ini sesuai dengan kelas & kelompok guru
if (!$jadwal || $jadwal['kelompok'] !== $guru_kelompok || $jadwal['kelas'] !== $guru_kelas) {
    echo '<div class="bg-red-100 p-4 rounded-lg">Anda tidak memiliki akses ke jadwal ini.</div>';
    return;
}

// Panggil semua helper yang dibutuhkan
require_once __DIR__ . '/../../../admin/helpers/fonnte_helper.php';
require_once __DIR__ . '/../../../admin/helpers/template_helper.php';
require_once __DIR__ . '/../../../admin/helpers/whatsapp_helper.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- PERBAIKAN: Tambahkan logika simpan kehadiran ---
        ...
        if (empty($kehadiran_data)) {
            ...
            try {
                ...
                foreach ($kehadiran_data as $rekap_id => $status) {
                    ...
                    // --- KIRIM NOTIFIKASI ALPA KE ORANG TUA ---
...
<div class="container mx-auto">
    <div class="mb-6"><a href="?page=jadwal" class="text-indigo-600 hover:underline">&larr; Kembali ke Daftar Jadwal</a></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-1 bg-white p-6 rounded-lg shadow-md">
            ...
            <form method="POST" action="?page=input_presensi&jadwal_id=<?php echo $jadwal_id; ?>">
                ...
                <div class="space-y-4">
                    <div><label class="block text-sm font-medium">Nama Pengajar*</label><input type="text" name="pengajar" value="<?php echo htmlspecialchars($jadwal['pengajar'] ?? $_SESSION['user_nama']); ?>" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required></div>
                        ...
                        <tbody id="presensiTableBody">
                            <?php foreach ($peserta_presensi as $peserta): $rekap_id = $peserta['id']; ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-3 space-y-2 sm:space-y-0 text-sm">
                                            <?php foreach (['Hadir', 'Izin', 'Sakit', 'Alpa'] as $status): ?>
                                                <label class="flex items-center">
                                                    <input type="radio" name="kehadiran[<?php echo $rekap_id; ?>]" value="<?php echo $status; ?>" class="h-4 w-4 status-radio" data-keterangan-id="keterangan-<?php echo $rekap_id; ?>" <?php echo ($peserta['status_kehadiran'] === $status) ? 'checked' : ''; ?>>
                                                    <span class="ml-1"><?php echo $status; ?></span>
                                                </label>
                                            <?php endforeach; ?>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 min-w-52">
                                        <input type="text" name="keterangan[<?php echo $rekap_id; ?>]" id="keterangan-<?php echo $rekap_id; ?>" value="<?php echo htmlspecialchars($peserta['keterangan'] ?? ''); ?>" class="block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
