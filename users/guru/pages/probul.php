...
// Jika session menyimpan nama kunci berbeda, sesuaikan key di atas
// misal: $_SESSION['kelompok'], $_SESSION['kelas']
...
// --- C. GET FILTER PERIODE ---
$filter_periode  = isset($_GET['periode_id']) ? (int)$_GET['periode_id'] : $default_periode_id;
$filter_kelompok = $guru_kelompok; // Dari session, tidak bisa diubah
$filter_kelas    = $guru_kelas;    // Dari session, tidak bisa diubah
...
<div class="container mx-auto space-y-6">
    ...
    <div class="bg-white p-5 md:p-6 rounded-lg shadow-md border-t-4 border-indigo-600">
        ...
        <form method="GET" action="">
            <input type="hidden" name="page" value="probul">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <!-- Kelompok: read-only dari session -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Kelompok</label>
                    <input type="text" value="<?php echo ucfirst(htmlspecialchars($guru_kelompok)); ?>"
                        class="block w-full bg-gray-100 border border-gray-300 rounded-lg py-2.5 px-3 sm:text-sm text-gray-500 cursor-not-allowed" readonly>
                </div>

                <!-- Kelas: read-only dari session -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Kelas</label>
                    <input type="text" value="<?php echo ucwords(htmlspecialchars($guru_kelas)); ?>"
                        class="block w-full bg-gray-100 border border-gray-300 rounded-lg py-2.5 px-3 sm:text-sm text-gray-500 cursor-not-allowed" readonly>
                </div>
            </div>

            <div class="mt-4">
                <button type="submit" class="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 md:py-2 px-6 rounded-lg shadow transition">
                    Tampilkan Target
                </button>
            </div>
        </form>
            ...
            <div class="px-5 md:px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center bg-gray-50 gap-3">
                <div>
                    ...
                    <p class="text-xs md:text-sm text-gray-500 mt-1">
                        Kelas: <span class="font-semibold text-indigo-700"><?php echo ucwords(htmlspecialchars($filter_kelas)); ?></span> |
                        Kelompok: <span class="font-semibold text-indigo-700"><?php echo ucwords(htmlspecialchars($filter_kelompok)); ?></span>
                    </p>
                </div>
...
<div id="modalTarget" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-75 hidden backdrop-blur-sm px-4">
    <div class="bg-white rounded-2xl md:rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100">
        ...
        <form id="formTarget" class="p-5 md:p-6 space-y-4 max-h-[85vh] overflow-y-auto custom-scrollbar">
            ...
            <input type="hidden" name="kelompok"   value="<?php echo htmlspecialchars($filter_kelompok); ?>">
            ...
            <div>
                ...
                <select name="master_materi_id" id="select_mapel"
                    class="w-full border border-gray-300 rounded-lg p-2.5 text-xs md:text-sm focus:ring-indigo-500 outline-none select-wrap-text" required>
                    <option value="">-- Pilih Materi Induk --</option>
                    <?php foreach ($list_mapel as $m): ?>
                        <option value="<?php echo $m['id']; ?>"
                            data-tipe="<?php echo $m['tipe_input']; ?>"
                            data-satuan="<?php echo $m['satuan_default']; ?>">
            ...
            <div>
                ...
                <select name="detail_materi_id" id="select_detail"
                    class="w-full border border-gray-300 rounded-lg p-2.5 text-[11px] md:text-sm focus:ring-indigo-500 outline-none select-wrap-text disabled:bg-gray-100"
                    disabled required>
    ...
    <div class="bg-white rounded-2xl md:rounded-xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all scale-100">
        ...
        <form id="formEdit" class="p-5 md:p-6 space-y-4 max-h-[85vh] overflow-y-auto custom-scrollbar">
            ...
            <div>
                ...
                <input type="text" name="judul_materi" id="edit_judul"
                    class="w-full border border-gray-300 rounded-lg p-2.5 text-[12px] md:text-sm font-semibold focus:ring-blue-500 outline-none" required>
            </div>
...
<script>
    // --- Prevent global overlay dari index.php ---
    ...
    // Saat mapel (induk) berubah → fetch detail
    if (selectMapel) {
        selectMapel.addEventListener('change', function() {
            ...
            fetch('pages/ajax_probul.php', { method: 'POST', body: formData })
                .then(res => res.json())
                .then(data => {
                ...
                .catch(() => {
                    selectDetail.innerHTML = '<option value="">Gagal memuat detail.</option>';
                });
        });
    }

    // Saat detail materi berubah → tampilkan form dinamis
    if (selectDetail) {
    ...
    if (formTarget) {
        formTarget.addEventListener('submit', function(e) {
            ...
            fetch('pages/ajax_probul.php', { method: 'POST', body: formData })
                ...
                .catch(() => { hideGlobalOverlay(); Swal.fire('Error', 'Sistem error', 'error'); });
        });
    }
